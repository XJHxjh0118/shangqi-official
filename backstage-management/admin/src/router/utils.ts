...
/** 从 localStorage 取出当前登录用户的菜单权限，过滤无权限的菜单 */
...
function hasMenuAccess(menuKey?: string, permissions: string[] = []) {
  if (!menuKey) return true;
  const perms = [...new Set(permissions.map(normalizeMenuKey))];
  ...
  if (!key.includes(":")) {
    return perms.some(p => p === key || p.startsWith(`${key}:`));
  }
  return false;
}
...
function filterNoPermissionTree(data: RouteComponent[]) {
  ...
  const newTree = cloneDeep(data).filter((v: any) =>
    hasMenuAccess(v.meta?.menu, currentPerms)
  );
  newTree.forEach(
